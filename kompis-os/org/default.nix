    ...
    { name, config, ... }@args:
    {
      options = (factories.mkEntity (args // { class = "app"; })) // {
        ...
        configurationFile = lib.mkOption {
          ...
          type = lib.types.coercedTo lib.types.str (s: ../../${s}) lib.types.path;
    ...
    { config, ... }:
    {
      options = {
        ...
        palette = lib.mkOption {
          description = "nested tailwind-like color mappings";
          type = with lib.types; attrsOf (attrsOf str);
          default = builtins.fromJSON (builtins.readFile ../../palette.json);
        };
        colors = lib.mkOption {
          ...
          default =
            let
              colorNames = builtins.attrNames config.palette;

              colorNameValuePairs = builtins.concatMap (
                colorName:
                let
                  shades = config.palette.${colorName};
                in
                map (shade: {
...
{
  options.flake = {
    org-pinned = lib.mkOption {
      description = "org.toml";
      type = lib.types.submodule orgModule;
      default = fromTOML (builtins.readFile inputs.org-toml);
    };
    org = lib.mkOption {
      description = "org.toml ";
      type = lib.types.submodule orgModule;
      default = fromTOML (builtins.readFile ../../org.toml);
    };
  };
}
